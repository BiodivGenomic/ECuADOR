...
#OUTPUT FILES DIRECTORIES
	...
	$tempDir = "cpDNA_regions_$output_files";										#SETUP DIRECTORY FILES
	$tempDir =~ s./.-.g;  
	$tempDir2 = "cpDNA_gff3_ext_$output_files";										#SETUP DIRECTORY FILES GBFF3 
	$tempDir2 =~ s./.-.g;  
	$tempDir3 = "cpDNA_fasta_ext_$output_files";									#SETUP DIRECTORY FILES FASTA
	$tempDir3 =~ s./.-.g; 
	$tempDir4 = "cpDNA_Sregions_$output_files";										#SETUP DIRECTORY FILES FASTA
	$tempDir4 =~ s./.-.g;
	$tempDir5 = "TcpDNA_oriented_$output_files";									#SETUP DIRECTORY FILES FASTA
	$tempDir5 =~ s./.-.g;   
...
opendir DH, $input or die "Cannot open directory: $!";								#OPEN CHLOROPLAST CONTAINER FOLDER
open (OUT, ">"."Summary_".$output_files.".txt"); 									#SETUP OUTPUT FILES SUMMARY FILE
open (OUT3, ">".$P_output_file);													#SETUP OUTPUT FILES PROBLEMATIC cpDNA FILES
...
while (my $file = readdir(DH)) {													#READ ALL FILES INTO THE FOLDER CONTAINER

	next if $file =~ /^\./; 														#REMOVE INVINCIBLES CHARACTERS FROM FOLDER
	...
	my $seqio = Bio::SeqIO->new('-file' => $new_file, '-format' => $format);		#GETTING AT THE FORMAT OBJECTS


	while($seqobj = $seqio->next_seq) { 											#RETRIEVE THE SEQ OBJECTS, ONE BY ONE
			
			$Count++;
			my $id1 = $seqobj->display_id;											#RETRIEVE ID
			my $id  = substr $id1, 0, 9; 											#REDUCE ID
			my $seq_seq     = $seqobj->seq();										#RETRIEVE SEQ
			if ($seq_seq =~ /^[A-Z\-]+$/) { 										#CHECK IF INPUT FILE CONTAIN A SEQUENCE
				...
				for (my $i = 1; $i <= $len - $winsize; $i += 1) {					#OPTIMIZE SLIDING WINDOW ACCORDING TO THE SIZE OF THE SEQUENCE
						$start = $i;												#SET START
						$end=$i +$winsize-1;										#SET END
						my	$seq_seq7 = $seqobj->subseq($start,$end);				#SLIDING WINDOW ANALYSIS
						my	$seq_seq8 = reverse_complement($seq_seq7);				##PERFORM REVERSE COMPLEMENT
						my	$seq_id7	 = "$id\_$start:$end";						#LABEL FRAGMENTS
						my	$seq_id8	 = "$id\_$start:$end";						##LABEL FRAGMENTS REVERSE COMPLEMENT
						push(@{$aHt7->{$seq_seq7}}, $seq_id7);
					
					if (defined $aHt7->{$seq_seq8}) {								#FIND FRAGMENT SIMILARITY
						$str = "@{$aHt7->{$seq_seq8}}_$seq_id8";					#BUILD A DYNAMIC SUFFIX ARRAY ACCORDING TO THE SIMILARITY FOUND
						#print $str."\n";			
											 
						if ($str =~ m/^[A-Z0-9\._.*?]+.*?_([0-9]+):([0-9]+)_[A-Z0-9\._.*?]+.*?_([0-9]+):([0-9]+)$/){	#MAIN SUFFIX ARRAY MATRIX PROCESSING

							$start1 .=$1."\n"; 										#SAVE  ELEMENTS FOR INITIAL FRAGMENTS IN ONE OF THE INVERTED REPEAT (IRa)
							$start2 .=$2."\n"; 										#SAVE  ELEMENTS FOR ENDING FRAGMENTS IN ONE OF THE INVERTED REPEAT (IRa)
							$start3 .=$3."\n";										#SAVE  ELEMENTS FOR INITIAL FRAGMENTS IN SECOND INVERTED REPEAT (IRb)
							$start4 .=$4."\n";										#SAVE  ELEMENTS FOR ENDING FRAGMENTS IN SECOND INVERTED REPEAT (IRb)
						}
					} 
				}

				@array1 = split /\n/, $start1;										#CONVERT ELEMENTS INTO AN ARRAY
				@array2 = split /\n/, $start2;
				@array3 = split /\n/, $start3;
				@array4 = split /\n/, $start4;	

				$s1= Set::IntSpan->new(@array1)."\n";								#SUMMARIZE ARRAY OF ELEMENTS FOR INITIAL FRAGMENTS (IRa)
				$s2= Set::IntSpan->new(@array2)."\n";								#SUMMARIZE ARRAY OF ELEMENTS FOR ENDING FRAGMENTS (IRa)
				$s3= Set::IntSpan->new(@array3)."\n";								#SUMMARIZE ARRAY OF ELEMENTS FOR INITIAL FRAGMENTS (IRb)
				$s4= Set::IntSpan->new(@array4)."\n";								#SUMMARIZE ARRAY OF ELEMENTS FOR INITIAL FRAGMENTS (IRb)

				my @arr = split /,/, $s1;											#COUNT NUMBER OF SEQUENCES FOUND FOR START IN IRA
				my @arr2 = split /,/, $s2;											#COUNT NUMBER OF SEQUENCES FOUND FOR END IN IRA
				my @arr3 = split /,/, $s3;											#COUNT NUMBER OF SEQUENCES FOUND FOR START IN IRB
				my @arr4 = split /,/, $s4;											#COUNT NUMBER OF SEQUENCES FOUND FOR END IN IRB
				...
				if ($s1 != "-" && $s2 != "-" && $s3 != "-" && $s4 != "-")  {		#INVETED REPEATS MATCHES
					if ($count==1) {												#IF MATCHES ARE IQUAL TO JUST ONE SERIE OF SEQUENTIAL ELEMENTS (NORMAL CASE)
						if ($count2 ==1 && $count3 ==1 & $count4 ==1) {

							$s1 =~ /^([0-9]+)-([0-9]+).*?\n$/; $min_ps1 =$1;		#EVALUATION OF NORMAL CASE (TWO INVERTED REPEATS)
							$s2 =~ /^([0-9]+)-([0-9]+).*?\n$/; $min_ps2 =$2;
							$s3 =~ /^([0-9]+)-([0-9]+).*?\n$/; $max_ps3 =$1;
							$s4 =~ /^([0-9]+)-([0-9]+).*?\n$/; $max_ps4 =$2;
							...
							if ($max_ps3<$min_ps2) {								#CHECK POINT-FRAGMENTS SIZE  
								...
								if ($min_ps1 == 1) {								#CHECK POINT FOR LSC REGION
								...
								$IRa = $seqobj->subseq($min_ps1,$min_ps2); 			#SEQUENCE REORDERING ACCORDING TO THE INVERTS REPEATED LOCATION 
								...
								if (length ($LSC2) > 1) {							#DETECTION OF TWO LSC REGIONS
									#print "LINE 326\n";
									...
									print OUT "Done\t"."Seq#".$Count."\t".$id."\t"."Total length: $len\t"."IRa_size: ".$size1."\t"."IRb_size: $size2"."\t"."LSC: 1-".length ($LSC)."\t"."IRa: ".$c."-".$a."\t"."SSC: ".($a+1)."-".$d."\t"."IRb: ".$b."-".$len."\n";
										...
										if ($format eq  "genbank") {				#GENBANK FILE PARSING - RECOVERY AND REORDERATION OF ANNOTATIONS IN A NEW SET OF COORDINATES
										...
										} elsif ($format eq "fasta") {				#EXTRACT REGIONS IN FASTA FILES
								...
								} elsif (length ($LSC2) == 1) {						#DETECTION OF ONE LSC REGION
										...
										if ($format eq  "genbank") {				#GENBANK FILE PARSING
											...
											@sdat1 = split /\n/, $dat1;				#SPLIT VARIABLES INTO AN ARRAY		
											@sdat2 = split /\n/, $dat2;
											@sdat3 = split /\n/, $dat3;
											@sm1 = split /\n/, $m1;
											@sm2 = split /\n/, $m2;
											...
											foreach $fsdat1 (@sdat1) {				#FILTER FOR GENBANK VARIABLES				
				  									$fsdat2= shift @sdat2;
				  									$fsdat3= shift @sdat3;
				   									$t2.=  "$id\tECuADOR\t".$fsdat3."\t".$fsdat1."\t".$fsdat2."\t.  +  .\tID=Done\tIs_circular=true\n";  
											...
											if ($out_gff3 == 1) {					#1RST CHECK POINT FOR GFF3 VARIABLES
										...
										} elsif ($format eq  "fasta") { 			#CHECK POINT FOR FASTA VARIABLES
											...
											if ($out_gff3 == 1) {					#2ND CHECK POINT FOR GFF3 VARIABLES
						...
						} elsif ($count != $count3 && $count2 != $count4) {			#CHECK POINT FOR SEVERAL REPEATS FOUND IN THE SEQUENCE
							print "Warning\t"."Seq#".$Count."\t".$id."\t"."Inadequate window size, increase sliding window option\n";
							print OUT3 "Warning\t"."Seq#".$Count."\t".$id."\t"."Inadequate window size, increase sliding window option\n";
						}


					} elsif ($count==2) {											#IF MATCHES ARE IQUAL TO TWO SERIES OF SEQUENTIAL ELEMENTS (TWO INVERTED REPEATS)

						if ($count2==2 && $count3==2 && $count4==2) {				#CHECK POINT FOR ALL MACHES FOUND

							$s2 =~ /^([0-9]+)-([0-9]+),(.*?)$/;
							$cs1= $3;
							my @ids = split/[-,]/, $s2;
							
							if (my @big_numbers = grep { $_ <= 21000 } @ids) { 		#DETECTION OF BROKEN INVERTED REPEATS

								my $scalar = join( ',' , @big_numbers );

								if (index($cs1, $scalar)) {							#MAIN SUFFIX MATRIX ARRAY PROCESSING
									...
									if ($max_ps3<$min_ps2) {						#CHECK POINT FOR SLIDING WINDOWS											
										print "Warning\t"."Seq#".$Count."\t".$id."\t"."Inadequate window size, increase sliding window option\n";
										print OUT3 "Warning\t"."Seq#".$Count."\t".$id."\t"."Inadequate window size, increase sliding window option\n";

									} elsif ($max_ps3>$min_ps2) {					#CHECK POINT FOR LSC REGIONS
										if ($min_ps1 == 1) {
											$LSC1 = $seqobj->subseq(1,$min_ps1);	
										} elsif ($min_ps1 != 1) {
											$LSC1 = $seqobj->subseq(1,$min_ps1-1);
											...
											$IRa = $seqobj->subseq($min_ps1,$min_ps2); #SUBSEQUENCES FOR MAIN CHLOROPLAST REGIONS ACCORDING TO THE SET OF COORDINATES 
											$SSC = $seqobj->subseq($min_ps2+1,$max_ps3-1); 
											$IRb = $seqobj->subseq($max_ps3,$max_ps4); 
											$LSC2 = $seqobj->subseq($max_ps4+1,$len);
											$LSC = $LSC2.$LSC1;
					...
					} elsif ($count>2) {											#IF MATCHES ARE GREATER THAN TWO SEQUENTIAL ELEMENTS (MORE THAN TWO INVERTED REPEATS)
						...
						if ($count == $count3 && $count2 == $count4) {
							...
							if (my @big_numbers = grep { $_ <= 26000 } @ids) {		#DETECT IF INVERTED REPEAT IS FRAGMENTED
